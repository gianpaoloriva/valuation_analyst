"""Script per analisi completa GOOGL (Alphabet Inc.).
...
COMPARABILI = [
    Comparabile(
        ticker="MSFT", nome="Microsoft Corporation", settore="Technology",
        market_cap=3_083_000, pe_ratio=31.7, ev_ebitda=22.7,
        pb_ratio=11.5, ev_sales=11.7, ev_ebit=26.0,
        margine_operativo=0.45, crescita_ricavi=0.16, paese="US",
    ),
    Comparabile(
    ...
    Comparabile(
        ticker="SNAP", nome="Snap Inc.", settore="Technology",
        market_cap=25_000, pe_ratio=None, ev_ebitda=85.0,
        pb_ratio=12.0, ev_sales=4.5, ev_ebit=None,
        margine_operativo=-0.02, crescita_ricavi=0.16, paese="US",
    ),
    Comparabile(
        ticker="TTD", nome="The Trade Desk Inc.", settore="Technology",
        market_cap=55_000, pe_ratio=140.0, ev_ebitda=75.0,
        pb_ratio=35.0, ev_sales=26.0, ev_ebit=80.0,
        margine_operativo=0.22, crescita_ricavi=0.26, paese="US",
    ),
    Comparabile(
        ticker="BIDU", nome="Baidu Inc.", settore="Technology",
        market_cap=38_000, pe_ratio=11.0, ev_ebitda=5.5,
        pb_ratio=0.9, ev_sales=2.0, ev_ebit=8.0,
        margine_operativo=0.18, crescita_ricavi=0.01, paese="CN",
    ),
]
...
def main() -> None:
    ...
    sezioni.append(f"> **Nota:** Alphabet ha una posizione di cassa netta di "
                  f"{formatta_miliardi(abs(DEBITO_NETTO) * 1e6)} (cassa > debito). "
                  f"Il peso del debito nella struttura del capitale e' minimo "
                  f"({formatta_percentuale(wacc_result.peso_debito)}), "
                  f"rendendo il WACC molto vicino al costo dell'equity.")
    sezioni.append("")
    ...
    sezioni.append("")
    sezioni.append(f"> **Nota:** Il CapEx di Alphabet ({formatta_miliardi(CAPEX * 1e6)}) "
                  f"e' molto elevato rispetto ai ricavi ({CAPEX/RICAVI:.1%}), "
                  f"riflettendo gli ingenti investimenti in data center e infrastruttura AI. "
                  f"Questo deprime il FCFF attuale ma dovrebbe generare ritorni futuri.")
    sezioni.append("")
    ...
    # Aggiungi GOOGL per confronto
    pe_googl = PREZZO_CORRENTE / EPS
    ev_ebitda_googl = ENTERPRISE_VALUE / EBITDA
    pb_googl = PREZZO_CORRENTE / BOOK_VALUE_PER_SHARE
    ev_sales_googl = ENTERPRISE_VALUE / RICAVI
    rows_comp.append([
        f"**{TICKER}**", f"**{NOME}**",
        f"**${MARKET_CAP/1000:,.0f}B**",
        f"**{pe_googl:.1f}**",
        f"**{ev_ebitda_googl:.1f}**",
        f"**{pb_googl:.1f}**",
        f"**{ev_sales_googl:.1f}**",
    ])
    ...
    sens_wacc_g = sensitivity_wacc_growth(
        ...
        debito_netto=DEBITO_NETTO,
        shares_outstanding=SHARES_OUTSTANDING,
        wacc_range=[0.08, 0.09, 0.095, 0.10, 0.105, 0.11, 0.12],
    ...
    sens_crescita_margine = sensitivity_crescita_margine(
        ...
        crescita_range=[0.05, 0.08, 0.10, 0.14, 0.18],
        margine_range=[0.25, 0.30, 0.32, 0.35, 0.40],
    )
    ...
    sezioni.append(f"- **Best Case** (25%): accelerazione AI (Gemini), crescita Cloud > 30%, margini in espansione (+35%)")
    sezioni.append(f"- **Base Case** (50%): continuazione trend attuale, crescita ricavi ~14%")
    sezioni.append(f"- **Worst Case** (25%): rallentamento ads, perdita quote ricerca, pressione regolatoria (-30%)")
    sezioni.append("")
    ...
    sezioni.append(f"- WACC: Distribuzione Normale (media={formatta_percentuale(wacc_val)}, std=1.2%)")
    sezioni.append("- Crescita Alta: Distribuzione Normale (media=14%, std=4%)")
    ...
    mc_result = monte_carlo_dcf(
        ...
        distribuzioni={
            "wacc": {
                "tipo": "normale",
                "media": wacc_val,
                "deviazione_standard": 0.012,
            },
            "crescita_alta": {
                "tipo": "normale",
                "media": CRESCITA_ALTA,
                "deviazione_standard": 0.04,
            },
            "crescita_stabile": {
                "tipo": "triangolare",
                "minimo": 0.015,
    ...
    sezioni.append("- Accelerazione dell'adozione di Gemini e AI products")
    sezioni.append("- Google Cloud supera il 15% di quota mercato cloud globale")
    sezioni.append("- YouTube diventa piattaforma dominante per streaming/TV")
    sezioni.append("- Miglioramento dei margini operativi tramite efficienza AI interna")
    sezioni.append("- Buyback aggressivi che riducono le azioni in circolazione")
    sezioni.append("")
    sezioni.append("### Rischi al Ribasso")
    sezioni.append("- Perdita di quote nel mercato della ricerca (AI competitors)")
    sezioni.append("- Pressione regolatoria antitrust (DOJ, UE)")
    sezioni.append("- Rallentamento del mercato pubblicitario digitale")
    sezioni.append("- CapEx AI superiore alle attese senza ritorni proporzionali")
    sezioni.append("- Competizione nel cloud da AWS e Azure")
    sezioni.append("- Rischio di disruption tecnologica nel core business Search")
    sezioni.append("")
    ...
    report_path = report_dir / f"GOOGL_valuation_report_{date.today().isoformat()}.md"
