...
- **Output**: `CashFlowProjection` con proiezione cash flow
- **Nota**: Gestisce anche FCFF negativo per aziende in perdita
...
- **Output**: Range di valori impliciti da multipli
- **Nota**: Salta automaticamente P/E e EV/EBITDA se negativi
...
Orchestrator --> Cost of Capital
             --> DCF Analyst (se possibile con FCFF negativo)
...
## Prerequisiti per gli Agenti

Prima di invocare qualsiasi agente su un ticker, il config deve esistere:

1. Creare `configs/{TICKER}.json` (copiare da `_template.json`)
2. Popolare i parametri dell'analista
3. La skill `/new-analysis TICKER` guida questo processo
